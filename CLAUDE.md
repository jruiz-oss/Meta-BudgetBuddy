...
- [x] **2026-09-21 (session 18 — Opus, with Blake)** — Budget-group correctness fixes + sheet-match diagnosis + per-account sheet refresh.
  ...
  - **`routes/pacing.py` — 0% members now report `action: "NO_ALLOCATION"`** with `recommended_daily_budget` 0 and `applyable: false`, instead of a $0 recommendation that Apply-all could push to Meta. `apply_recommendations` also refuses any CBO adjustment whose campaign has an explicit 0% group allocation — belt and braces, since the UI is not the only caller.
  - **`routes/pacing.py` — flight-excluded group members are now fetched for spend.** `_campaign_should_run_today` drops ended flights from the run, but a group's recommendation is `group budget − COMBINED group spend`. Dropping an ended member overstated what was left and over-funded the live ones. New `spend_only` list feeds `group_spend_map` without producing recommendations. Skipped for single-campaign runs.
  - **`routes/sheets.py` — `_campaign_mtd_spend` had no month filter.** Both branches took the most recent `PacingData` row by date, *whatever month it was from*, and write-back pushed that into the current tab as MTD spend. Harrah's Ak-Chin's Promo row read $3,118 when September spend was $579 — the difference was six retired campaigns reporting their May/June/July totals (Mother's Day alone contributed $911.79). Now filtered to `date >= _current_month_start()`; a campaign with no current-month row correctly reports "no pacing data yet".
  - **`routes/sheets.py` — preview now explains itself.** New `_match_diagnostics()` returns a plain-language reason plus the three closest-scoring campaign names with scores. `GET /api/sheets/<id>/preview` includes `reason` and `candidates` on every non-match, covering: column-D scope mismatch, prefix routed elsewhere, substring ambiguity, below-threshold score, score ties, aborted splits (naming the chunk that failed), and splits with no budget in col B. Notes that fail to parse as a split get an explicit hint about the `/` chunk separator.
  - **`AccountDashboard.jsx`** — two new buttons. **Refresh from sheet** calls `POST /api/sheets/<id>/sync-budgets` for this account only: re-reads budgets, allocations and notes with no Meta calls and no pacing, so it can't hit rate limits or touch other accounts. **Sheet match** opens a read-only modal listing every row on the current tab, what it resolved to, and why it didn't. The "No sheet match" chip in the recommendations table is now clickable and opens the same modal.
  - **`Home.jsx`** — new **Ads Manager** button beside Dashboard on each account header. Deep-links to `adsmanager.facebook.com` for that `act=` with the date range set to month-start → **yesterday** (matching pacing's window, which is why Ads Manager's default totals read higher) and `filter_set` limited to campaigns whose name contains "Commit". The filter uses Meta's record-separator format (`SEARCH_BY_CAMPAIGN_GROUP_NAME-STRING_SET <U+001E> CONTAINS_ALL <U+001E> ["commit"]`), URI-encoded. `routes/campaigns.py` now includes `meta_account_id` in the `/api/campaigns/all` account payload to support it. Opens in a new tab; no `business_id` is sent, so Ads Manager resolves scope from the signed-in session.
  - **`Home.jsx` — campaigns are now grouped by budget group.** Members of one sheet row's split (e.g. "Harrah's Ak-Chin - Core" covering Summer Playfest / Free Slot Play / Sports Bar) sit together under a header row showing the sheet row name, the combined budget and the combined MTD spend — the two numbers the group recommendation is actually computed from. Members sort by allocation % descending, so the primary campaign leads and 0% spend-only members sink to the bottom. Ungrouped campaigns follow the groups. `Campaign.to_dict` gained `budget_group_name` and `budget_group_total`, and both campaign-list queries eager-load `Campaign.budget_group` so the extra fields don't reintroduce an N+1.
  - **`Home.jsx`** — 0% group members render as "Spend only" with no Apply button and are excluded from the actionable count.
...
- [x] **2026-05-26 (session 16 — Opus)** — Global Meta token is now workspace-shared.
  ...
  - **`backend/routes/accounts.py`** — `global_token()` rewritten. GET no longer reads the current user's row; instead `_workspace_token()` scans every `User` and returns the first non-empty `global_meta_token`. PUT now propagates the new value to **every** User row (single transaction). Net effect: any teammate sees the same `has_token` / `preview`, and the token survives the original-setter being deleted.
  - **`backend/routes/auth.py`** — `register()` now seeds the new user's `global_meta_token` from any existing user's token before the first commit. Coworkers who sign up after Jorge sets a token start with it already populated — no extra step required from them.
  - **`backend/database.py`** — `Account.effective_meta_token` got a safety-net fallback. Order is unchanged for normal accounts (account-level override → linking user's global → ''), but now there's a third hop: any user's global token. Covers (a) accounts linked by a user whose row pre-dates the propagation logic, and (b) the original token-setter being deleted.
  - ⚠️ **No DB migration required.** Pure code change — `global_meta_token` is still a per-User encrypted column, but the column is now used redundantly to store a workspace-shared value.
  - ⚠️ **Behavior change worth knowing.** The global token is now genuinely workspace-shared. If any teammate changes it (or clears it), it changes/clears for everyone on the next page load. That matches the rest of the session-13 shared-workspace model but it's a heads-up worth giving the team before deploy.
  - **Migrating Jorge's existing token to the new model:** after deploying, open the Accounts page, hit Save on the global-token field with the same value re-pasted. The PUT now writes it to every existing User row in one commit. Future signups inherit it via the register seed.
  - Verified: `python3 -c "import ast; ast.parse(...)"` clean across `routes/accounts.py`, `routes/auth.py`, `database.py`.
